...
import {
  ooxmlAppliedOperationSchema,
  ooxmlUnsupportedOperationSchema,
  ooxmlUnsupportedReasonCodeSchema,
  pptxOoxmlSyncPayloadSchema,
  pptxOoxmlSyncWorkerResponseSchema,
  slideMotionSyncInputSchema,
  unsupportedSlideMotionSchema,
  type OoxmlMotionOperation,
  type OoxmlSyncOperation,
  type PptxOoxmlSyncWorkerResponse,
  type SlideMotionSyncInput,
} from "./pptx-ooxml-sync/contracts";
...
class UnsupportedOoxmlSlideMotionError extends Error {
  constructor(readonly motion: z.infer<typeof unsupportedSlideMotionSchema>) {
...
function withSyncResult(
  ...
  return templateBlueprintSchema.parse({
    ...
    slides: currentSlides.map((slide, index) => {
      const notesRenderAssetFileId =
        assets.notesRenderAssetFileIdsByAssetId.get(
