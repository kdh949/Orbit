...
import { analyzeKoreanFillers } from "@orbit/shared/slide-practice";
import {
  legacyRehearsalSlideSpeakingRate,
  rehearsalAnalyzeRequestSchema,
} from "@orbit/shared/coaching";
import { type Deck, deckPatchSchema, deckSchema } from "@orbit/shared/deck";
...
import {
  analyzeMissedKeywordSchema,
  analyzeResponseSchema,
  analyzeSemanticResponseSchema,
  audioAssetRowSchema,
  buildSlideSpeakingRateBusinessEvent,
  deckPatchRowSchema,
  deckRowSchema,
  rehearsalRunInputRowSchema,
  rehearsalSttPayloadSchema,
  transcriptBlockingReasons,
  type AudioAssetRow,
  type RehearsalSemanticEvaluationBusinessEvent,
  type RehearsalSilenceAnalysisBusinessEvent,
  type RehearsalSlideSpeakingRateBusinessEvent,
  type RehearsalSttPayload,
  type RehearsalTranscriptArtifactBusinessEvent,
  type SemanticAnalysisResult,
} from "./rehearsal-stt/contracts";
export type {
  RehearsalSemanticEvaluationBusinessEvent,
  RehearsalSilenceAnalysisBusinessEvent,
  RehearsalSlideSpeakingRateBusinessEvent,
  RehearsalTranscriptArtifactBusinessEvent,
...
function applySlideTranscriptFillerAnalysis(
  ...
  for (const snapshot of snapshots) {
    ...
    for (const detail of fillers.details) {
      wordCounts.set(
        detail.word,
        (wordCounts.get(detail.word) ?? 0) + detail.count,
      );
    }
