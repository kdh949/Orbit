...
- program-v2의 visible 고채도 accent는 `focal`, `secondary` 두 role로 제한한다. compiler가 추가 accent를 두 role 중 가까운 색으로 정규화하고, 실제 element에 사용되지 않은 고채도 role은 최종 `designProgramSnapshot.paletteRoles`에서 사용 중인 role 또는 text role로 치환한다.
- 한글 typography는 browser 품질 추정과 PPTX export가 같은 `fontWidthFactor`, `lineHeight`, `pptxFontFamily`, `fallbackFontFamily` resolver를 사용한다. text element는 optional `fontWidthFactor`를 보존하고 PPTX run은 `a:ea` East Asian typeface를 명시한다.
...
- 기존 Deck은 새 필드 없이 parse된다. System Design Pack 결과는 기존 `metadata.designProgramSnapshot`에 optional `designPackId`, `designPackVersion`, `selectionMode`, `selectionReason`, `selectionFallbackUsed`, `layoutIds`, `layoutCatalogVersion`을 추가해 재현한다. auto selector는 active이면서 provenance가 approved인 pack만 사용하고, 같은 input과 `layoutCatalogVersion`에 대해 안정적인 ID tie-break로 같은 결과를 선택한다. 명시적인 System Design Pack과 Saved Design Pack preference는 auto 선택보다 우선한다.
...
- staged `program-v2`는 slide shard에서 최대 2회의 bounded repair를 허용하고, 모든 shard join 이후 slide PNG 전체와 montage를 새로 렌더링해 whole-deck review를 수행한다. repair 예산을 소진한 P1은 승인된 no-media safe layout으로 한 번 재매핑한 뒤 전체 Deck을 다시 검토한다.
...
- `slides[].elementSources[]`: `{ elementId, elementType?, ooxmlOrigin?, ooxmlEditCapabilities?, slidePart, shapeId, relationshipId?, sourceType, writable, tableCellLocators?, chartDataLocator?, fallbackReason? }`. 신규 importer/sync 결과는 `elementType`을 기록하며, 이 값이 없거나 실제 patch 대상 type과 다르면 property sync를 fail-closed한다.
...
- chart source의 optional `chartDataLocator`는 `{ chartType, chartPart, workbookPart, workbookFingerprint, categoryFormula, series[] }`이며 `series[]`는 `{ titleFormula, valueFormula }`를 가진다. `ooxmlEditCapabilities.chartData=true`는 imported writable `chart` source가 단일 internal chart relationship, 지원 chart type, direct absolute formula range, 단일 internal `.xlsx` workbook과 그 lowercase SHA-256 fingerprint를 모두 증명한 경우에만 허용한다. generic imported chart에는 이 capability를 추정해 부여하지 않는다.
...
- `content-planning` artifact는 공개 가능한 `{order,title}` outline만 별도 투영하고 Worker는 `render-validation`의 각 render asset을 zero-padded `slide-render` shard로 저장한다. source text, slot content와 private locator는 preview artifact에 넣지 않는다.
...
Worker publication 경계는 요청의 exact `templateSelection.templateId@version`과 materialization의 top-level 및 bounded Job result `templateSnapshot.catalogTemplateId@catalogTemplateVersion`을 다시 비교한다. 하나라도 다르면 publication artifact, Deck, TemplateBlueprint, package asset을 쓰기 전에 `OOXML_REFERENCE_PUBLICATION_IDENTITY_MISMATCH`로 terminal fail-closed한다.

Python non-2xx 응답의 `{detail:{code,retryable}}`는 8 KiB 이하 strict JSON과 128자 이하 `OOXML_REFERENCE_*` code일 때만 Worker Job error로 보존한다. 그 외 body와 provider detail은 폐기하고 generic bounded code를 사용한다.
...
기능 flag와 exact allowlist가 켜진 Python runtime은 private storage의 versioned fidelity calibration object를 시작 시 checksum 검증한다. calibration은 strict schema version, `status=calibrated`, exact 7개 template@1 identity baseline, 하나의 renderer/version, locked-region threshold, `geometryEdgeTolerancePx=0`, 승인 근거와 exact approved font-alias policy를 모두 요구한다. object 부재·checksum drift·renderer baseline 또는 alias policy 불완전은 runtime을 fail-closed하며 object locator는 response와 로그에 노출하지 않는다. disabled template-manifest version 2 image proposal은 현재 calibration 대상이 아니며, 활성화하려면 exact version 2 baseline과 calibration 계약을 새로 고정한다.

generated fidelity는 intended slot pixel mask와 별개로 모든 shape의 frame(`x`,`y`,`cx`,`cy`, rotation, flip, z-order), mutable slot content를 제거한 locked style, slide→layout→master→theme part identity/checksum을 비교한다. slide shape에 `a:xfrm`이 없는 placeholder frame은 `p:ph` identity로 slide→layout은 exact `idx`, layout→master는 OOXML placeholder 의미 type(`ctrTitle→title`, content 계열→`body`)의 unique match를 상속한다. 상속 관계가 누락되거나 모호하면 mask와 locked snapshot을 만들지 않고 `OOXML_REFERENCE_SLOT_MASK_UNAVAILABLE` 또는 package validation error로 fail-closed한다. deterministic renderer provenance는 package가 요청한 모든 explicit font family가 실제 resolve된 file checksum을 기록하며 일반 substitution은 허용하지 않는다. LibreOffice 변환은 호출마다 fresh `UserInstallation` profile을 사용해 기존 GUI/headless instance의 profile 또는 process environment를 재사용하지 않는다. `Lora SemiBold`, `Roboto SemiBold`, `Roboto Serif 14pt`, `Roboto Serif 14pt Medium`만 2026-07-23 승인된 v1 alias policy로 각각 `Lora/SemiBold/wght=600`, `Roboto/SemiBold/wdth=100,wght=600`, `Roboto Serif/Regular/GRAD=0,opsz=14,wdth=100,wght=400`, `Roboto Serif/Medium/GRAD=0,opsz=14,wdth=100,wght=500`에 매핑할 수 있다. 이 예외는 `target="pattern"` fontconfig rewrite를 OOXML reference render와 font-match subprocess에만 적용하고 일반 GenerateDeck/System Design Pack/PPTX import renderer의 fontconfig에는 주입하지 않는다. target family/style, family-style의 실제 fontconfig weight/width, variable axis, OFL checksum, font file SHA-256과 canonical policy SHA-256이 모두 일치할 때만 `approved-alias`로 통과한다. alias 누락·추가, checksum/axis/style drift, `target="scan"` 또는 name-table 변조는 실패한다. public API와 report에는 font path나 bytes를 포함하지 않는다.
...
`TextElementRun.letterSpacing`은 optional finite canvas unit이며 DrawingML `a:rPr@spc`를 importer, editor measure/render, OOXML sync와 generic export가 보존한다. text-content slot 편집은 문자 간격을 변경 권한으로 추가하지 않고 기존 run style을 유지한다.
...
reference-template chart의 imported targeted sync는 authoritative source mapping의 `chartData=true`와 live `chartDataLocator`를 매 요청 다시 검증한다. patch는 기존 chart type에서 category/series 값만 바꿀 수 있고 category count, series count와 formula range는 고정한다. Python은 chart XML cache와 embedded workbook을 한 package 결과로 함께 갱신하고 slide frame, chart style, geometry와 relationship은 보존한다. 성공 응답은 새 workbook SHA-256을 `elementSources[].chartDataLocator.workbookFingerprint`에 기록한다. chart type/count/formula/relationship/workbook fingerprint drift 또는 외부 workbook은 `CHART_*` reason code로 package bytes, asset, blueprint freshness를 모두 원본 상태로 유지하며 fail-closed한다.
...
- `CHART_CAPABILITY_UNSAFE`, `CHART_FORMULA_DRIFT`, `CHART_RELATIONSHIP_UNSAFE`, `CHART_STRUCTURE_UNSUPPORTED`, `CHART_TYPE_UNSUPPORTED`, `CHART_WORKBOOK_FINGERPRINT_MISMATCH`, `CHART_WORKBOOK_UNSUPPORTED`
