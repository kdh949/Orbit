...
generated fidelity는 intended slot pixel mask와 별개로 모든 shape의 frame(`x`,`y`,`cx`,`cy`, rotation, flip, z-order), mutable slot content를 제거한 locked style, slide→layout→master→theme part identity/checksum을 비교한다. slide shape에 `a:xfrm`이 없는 placeholder frame은 `p:ph` identity로 slide→layout은 `idx`, layout→master는 explicit `type`을 우선해 상속하고 unique fallback만 허용한다. 상속 관계가 누락되거나 모호하면 mask와 locked snapshot을 만들지 않고 `OOXML_REFERENCE_SLOT_MASK_UNAVAILABLE` 또는 package validation error로 fail-closed한다. deterministic renderer provenance는 package가 요청한 모든 explicit font family가 실제 resolve된 file checksum을 기록하며 substitution은 허용하지 않는다.
