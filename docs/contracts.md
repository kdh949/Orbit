...
generated fidelity는 intended slot pixel mask와 별개로 모든 shape의 frame(`x`,`y`,`cx`,`cy`, rotation, flip, z-order), mutable slot content를 제거한 locked style, slide→layout→master→theme part identity/checksum을 비교한다. deterministic renderer provenance는 package가 요청한 모든 explicit font family가 실제 resolve된 file checksum을 기록하며 substitution은 허용하지 않는다.
