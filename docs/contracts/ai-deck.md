...
- `develop` merge는 `.github/workflows/build-personal-staging-images.yml`에서 Web/API/Worker/Python worker 이미지를 동일 commit SHA로 빌드해 GHCR에 게시한다. workflow는 개인 서버에 접속하거나 컨테이너를 교체하지 않는다. 관리자는 성공한 SHA를 선택해 Sophos VPN/관리 LAN의 SSH 경로에서 수동 배포한다.
- #339 종료 증거는 image workflow의 성공 SHA, 서버에서 확인한 `git rev-parse HEAD`, 수동 배포 후 BullMQ `pptx-import`, `ai-template-deck-generation`, `generate-deck` 전체 상태와 관련 DB Job의 `queued`/`running`을 읽기 전용으로 확인하고 GenerateDeck smoke를 실행한 결과다. workflow SHA와 실제 서버 HEAD를 구분하며, 성공한 image build만으로 배포·queue/DB 상태를 주장하지 않는다. production의 ingress 중단, drain, 동시 교체와 cache invalidation은 별도 승인된 배포 계획에서 다룬다.
