...
| Task 8 fidelity harness | 자동 통과, identity baseline 생성 | inherited placeholder frame, slot frame/relationship known drift, package font exact resolve, private calibration fail-closed test, PowerPoint 7개/139장 identity diff와 7개/56장 artifact runner | exact font environment와 threshold 사람 승인 |
| Checkpoint B2 | actual text-slot 기계 검증 및 montage 생성 통과, 승인 보류 | 7개 sync/export warning 0, Python/LibreOffice/PowerPoint reopen과 text 유지, `/private/tmp/orbit-ooxml-b2-slot-montage-7cg9oi8q`의 43개 checksum/structural 검증 | operating 42px·simple-light 72px locked diff 사람 판정, production publication과 사람 검수 |
...
| Task 20 flag/runbook/E2E | 자동 통과 | exact `template-id@version` config/API test, flag off/on `/createdeck` Playwright 2 tests, generation→제한 편집→sync gate→export mock product path | approved calibration object가 있는 current-branch Compose flag-on actual source smoke와 운영 지표 관찰 |
| Checkpoint D2 | 부분 통과, 승인 보류 | build 10/10, lint/test 17/17, Python 1,035, PostgreSQL PPTX round-trip 7/7, `/createdeck` Chrome 2/2, flag off/allowlist rollback, actual 7개 edit/export PowerPoint/LibreOffice와 7개/56장 fidelity artifact | real API→queue→publication E2E, production storage, requested exact font와 사람 승인 |
