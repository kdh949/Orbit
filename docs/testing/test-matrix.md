...
| Task 3 private catalog | local QA source/preview checksum 검증, strict manifest 미게시, repository 전체 disabled | registry/catalog v2 approval/blocker test, local QA source 7개+preview 139개=146-object read-after-write/anonymous 403와 승인 후 decision artifact SHA-256 | strict manifest publication, production managed storage와 exact font QA |
...
| Checkpoint A | inventory·annotation 사람 검수 충족, 승인 보류 | 7개/139장, strict schema/regression, 승인/disabled repository catalog, local QA 146-object checksum/private ACL과 `/private/tmp/orbit-ooxml-private-catalog-decision-disabled-20260723` | strict manifest와 §15 production private managed storage publication |
...
| Task 8 fidelity harness | fixture 검증 통과, threshold acceptance 미완료 | inherited placeholder frame, known drift, private calibration fail-closed test, PowerPoint 7개/139장 identity diff와 `/private/tmp/orbit-ooxml-identity-calibration-candidate-20260723-b`의 LibreOffice 7개/139장 SSIM 1.0·changed pixel/structural/package drift 0·461 checksum | exact font environment 재측정과 threshold 사람 승인 |
| Checkpoint B2 | actual text-slot 기계 검증 및 montage 생성 통과, 승인 보류 | `/private/tmp/orbit-ooxml-powerpoint-slot-roundtrip-bodypr-fixed-20260723-a`의 7개 sync/export PowerPoint reopen, `/private/tmp/orbit-ooxml-b2-slot-montage-7cg9oi8q`, `/private/tmp/orbit-ooxml-actual-text-slot-matrix-v2-20260723-60dgia3v`의 253/253 `bodyPr`/style/non-text semantics/locked structure gate 통과 | actual image annotation, operating 42px·simple-light 72px locked diff 사람 판정, production publication과 사람 검수 |
...
| Checkpoint C | 자동 검증 7/7 통과, strict 상태 `failed` | 7개 actual full-deck sequence/package/PowerPoint/LibreOffice, `/private/tmp/orbit-ooxml-powerpoint-slot-roundtrip-bodypr-fixed-20260723-a`의 actual slot sync/reopen과 `/private/tmp/orbit-ooxml-fidelity-artifacts-20260723-g`; 351개 resolved font checksum을 기록했지만 strict evidence는 `FONT_AVAILABILITY_VALIDATION_PENDING` 유지 | requested exact font 설치/checksum, production storage와 사람 fidelity 승인 |
...
| Task 14 catalog/preview API | ephemeral active-manifest fixture 통과, deployed catalog 전체 disabled | catalog projection/auth/checksum/preview-missing test와 Python option projection | local QA/production strict manifest publication |
...
| Checkpoint D2 | 부분 통과, 승인 보류 | build 10/10, lint/test 17/17, Python 1,038 passed/1 skipped, PostgreSQL PPTX round-trip 7/7, isolated Compose flag-off health/readiness, route-mocked `/createdeck` Chrome 2/2와 no-mock 1 skipped, actual 7개 edit/export PowerPoint/LibreOffice와 7개/56장 fidelity artifact | flag-on/allowlist rollback actual runtime, no-mock spec의 real API→queue→publication 실주행, production storage, requested exact font와 사람 승인 |
