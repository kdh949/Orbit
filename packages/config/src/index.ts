...
const commaSeparatedStringSchema = z.preprocess(
  (value) => {
...
const usesHttpProtocol = (url: string): boolean =>
...
export const orbitEnvSchema = z
  .object({
    ...
    API_TRUST_PROXY_HOPS: optionalIntegerInRange(
    ...
    PRIVATE_EVIDENCE_REDIS_URL: requiredString(
      "PRIVATE_EVIDENCE_REDIS_URL",
    ).default(localDefaults.PRIVATE_EVIDENCE_REDIS_URL),
    ...
    LOAD_TEST_MODE: booleanStringSchema.default(false),
    LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN: optionalString.pipe(
      z
        .string()
        .min(
          32,
    ...
    LOAD_TEST_PROVIDER_MODE: z
      .enum(["disabled", "deterministic"])
      .default("disabled"),
    LOAD_TEST_PROVIDER_SEED: optionalIntegerInRange(
      "LOAD_TEST_PROVIDER_SEED",
      42,
      0,
      2_147_483_647,
    ),
    LOAD_TEST_PROVIDER_DELAY_MS: optionalIntegerInRange(
      "LOAD_TEST_PROVIDER_DELAY_MS",
      100,
      0,
      60_000,
    ),
    LOAD_TEST_PROVIDER_ERROR_RATE_PERCENT: optionalIntegerInRange(
      "LOAD_TEST_PROVIDER_ERROR_RATE_PERCENT",
      0,
      0,
      100,
    ),
    LOAD_TEST_PROVIDER_PAYLOAD_BYTES: optionalIntegerInRange(
    ...
    COACHING_IDEMPOTENCY_HMAC_KEY_VERSION: optionalIntegerInRange(
    ...
    COACHING_IDEMPOTENCY_HMAC_PREVIOUS_KEY_VERSION: z.preprocess(
      (value) =>
        value === undefined || value === null || value === ""
          ? undefined
          : Number(value),
      z.number().int().min(1).max(32767).optional(),
    ),
    ...
    PUBLIC_IMAGE_PROVIDER: z
      .enum(["disabled", "openverse"])
      .default("openverse"),
  ...
  .superRefine((value, context) => {
    const loadTestConfigured =
      value.LOAD_TEST_MODE ||
      value.LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN !== undefined ||
      value.LOAD_TEST_PROVIDER_MODE === "deterministic";
    if (value.APP_ENV === "production" && loadTestConfigured) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["LOAD_TEST_MODE"],
        message: "Load-test mode is forbidden in production",
      });
    }
    if (value.LOAD_TEST_MODE && !value.LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN"],
        message:
          "LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN is required when LOAD_TEST_MODE=true",
      });
    }
    if (value.LOAD_TEST_RATE_LIMIT_BYPASS_TOKEN && !value.LOAD_TEST_MODE) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["LOAD_TEST_MODE"],
        message:
    ...
    if (
      (value.FOCUSED_PRACTICE_ENABLED || value.CHALLENGE_QNA_ENABLED) &&
      !value.ADAPTIVE_REHEARSAL_COACH_ENABLED
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["ADAPTIVE_REHEARSAL_COACH_ENABLED"],
        message:
          "Adaptive coaching core must be enabled before focused practice or Challenge Q&A",
      });
    }
    if (value.APP_ENV === "production" && value.DEMO_COACHING_FIXTURE_ENABLED) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["DEMO_COACHING_FIXTURE_ENABLED"],
        message: "Demo coaching fixtures are forbidden in production",
      });
    }
    if (
    ...
    if (
      value.DEMO_AI_DECK_CACHE_ENABLED &&
      !value.DEMO_AI_DECK_SOURCE_PROJECT_ID
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["DEMO_AI_DECK_SOURCE_PROJECT_ID"],
        message:
          "Demo AI deck source project is required when the cache is enabled",
      });
    }
    if (value.DEMO_AI_DECK_CACHE_ENABLED && !value.DEMO_AI_DECK_TRIGGER_TOPIC) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["DEMO_AI_DECK_TRIGGER_TOPIC"],
        message:
          "Demo AI deck trigger topic is required when the cache is enabled",
      });
    }
    if (
      value.APP_ENV === "production" &&
      value.ADAPTIVE_REHEARSAL_COACH_ENABLED &&
      (!value.COACHING_IDEMPOTENCY_HMAC_SECRET ||
        value.COACHING_IDEMPOTENCY_HMAC_SECRET.length < 32)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["COACHING_IDEMPOTENCY_HMAC_SECRET"],
        message:
          "Production coaching idempotency HMAC secret must be at least 32 characters",
      });
    }
    const hasPreviousSecret = Boolean(
      value.COACHING_IDEMPOTENCY_HMAC_PREVIOUS_SECRET,
    );
    const hasPreviousVersion =
      value.COACHING_IDEMPOTENCY_HMAC_PREVIOUS_KEY_VERSION !== undefined;
    if (hasPreviousSecret !== hasPreviousVersion) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["COACHING_IDEMPOTENCY_HMAC_PREVIOUS_SECRET"],
        message:
          "Previous coaching HMAC secret and key version must be configured together",
      });
    }
    if (
      hasPreviousVersion &&
      value.COACHING_IDEMPOTENCY_HMAC_PREVIOUS_KEY_VERSION ===
        value.COACHING_IDEMPOTENCY_HMAC_KEY_VERSION
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["COACHING_IDEMPOTENCY_HMAC_PREVIOUS_KEY_VERSION"],
        message:
          "Previous coaching HMAC key version must differ from the current version",
      });
    }
    if (value.PRIVATE_EVIDENCE_REDIS_URL === value.REDIS_URL) {
      context.addIssue({
        ...
        message:
          "PRIVATE_EVIDENCE_REDIS_URL must use a separate non-persistent Redis instance",
      });
    }
    ...
    if (
      ...
      (!usesHttpProtocol(value.WEB_ORIGIN) ||
        !usesHttpProtocol(value.API_BASE_URL))
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["AUTH_COOKIE_SECURE"],
...
export function isAdaptiveCoachingProjectAllowed(
  config: OrbitConfig,
  projectId: string,
) {
  return (
    config.ADAPTIVE_COACHING_PROJECT_ALLOWLIST.includes("*") ||
    config.ADAPTIVE_COACHING_PROJECT_ALLOWLIST.includes(projectId)
  );
}
